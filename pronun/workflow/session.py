...
class Session:
    ...
    def __init__(
        ...
        # Load trained emission parameters if available
        self._emission_store = None
        try:
            self._emission_store = EmissionStore.from_file(EMISSION_STORE_PATH)
        except (FileNotFoundError, Exception):
            pass

        # Load calibrated reference baseline if available
        sys.stderr.write(f"[DEBUG] Loading baseline from: {REFERENCE_BASELINE_PATH} (exists={REFERENCE_BASELINE_PATH.exists()})\n")
        sys.stderr.flush()
        try:
            self.visual_scorer.reference.load(str(REFERENCE_BASELINE_PATH))
            sys.stderr.write(f"[DEBUG] Loaded baseline: default_ref={self.visual_scorer.reference.default_reference:.2f}, bandwidth={self.visual_scorer.reference.bandwidth:.2f}, words={len(self.visual_scorer.reference._references)}\n")
            sys.stderr.flush()
        except FileNotFoundError:
            sys.stderr.write("[DEBUG] Baseline file not found, using defaults\n")
            sys.stderr.flush()
        except Exception as e:
            sys.stderr.write(f"[DEBUG] Failed to load reference baseline: {e}\n")
            sys.stderr.flush()
    ...
    def _compute_visual_score(self, video_frames, text):
        ...
        emission_dict = self._emission_store.to_dict() if self._emission_store else {}

        if self.mode in ("B", "BOTH"):
            viseme_seq = self.lee_viseme.text_to_viseme_sequence(text)
            hmm = self.visual_scorer.build_hmm(
                viseme_seq, emission_dict, obs.shape[1],
            )
            score_b = self.visual_scorer.score(hmm, obs, text)
            visual_score_b = score_b["score"]
            print(f"[DEBUG visual] L_norm={score_b['log_likelihood_norm']:.2f}  L_ref={score_b['reference']:.2f}  diff={score_b['log_likelihood_norm'] - score_b['reference']:.2f}  score={visual_score_b:.2f}")

        if self.mode in ("A", "BOTH") and self.kmeans_model:
            pred_visemes = self.kmeans_model.predict(obs)
            hmm = self.visual_scorer.build_hmm(
                list(pred_visemes), emission_dict, obs.shape[1],
            )
